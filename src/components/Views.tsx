  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap justify-between items-center gap-3">
        ...
        {onOpenRadioCall && (
          <button
            onClick={() => onOpenRadioCall({ type: 'Channel', id: activeChannel?.id || 'inc-015', name: activeChannel?.name || 'Tactical Net', channel: activeChannel })}
            className="px-3.5 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs flex items-center gap-2 shadow-[0_0_15px_rgba(16,185,129,0.3)] transition-all"
          >
            <PhoneCall size={14} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
        ...
        <div className="lg:col-span-4 space-y-4">
          ...
          <div className="bg-[#0b101d] border border-[#172338] rounded-2xl p-4 shadow-sm space-y-3">
            <div className="flex items-center justify-between border-b border-[#172338] pb-2.5">
              <span className="text-xs font-bold text-slate-200">Channel Telemetry</span>
            ...
            <div className="space-y-2 text-xs">
              <div className="bg-[#090e1a] p-2.5 rounded-xl border border-[#1a2942] space-y-1">
                <span className="text-[10px] text-slate-500 font-bold uppercase">Net Name</span>
                <p className="font-bold text-slate-100">{activeChannel?.name}</p>
              ...
              {targetIncident && (
                <div className="bg-[#090e1a] p-2.5 rounded-xl border border-[#1a2942] space-y-1">
                  ...
                  <div className="flex justify-between items-center">
                    <span className="font-bold text-red-400 font-mono">{targetIncident.id}</span>
                    <span className="text-slate-300 font-medium">{targetIncident.location}</span>
                  </div>
                </div>
              )}
            </div>
          ...
          <div className="bg-[#0b101d] border border-[#172338] rounded-2xl p-4 shadow-sm space-y-3">
            ...
            <div className="space-y-2 max-h-72 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-800">
              {vehicles.slice(0, 8).map(v => {
                const unitStyles = getUnitStyles(v.type || v.id);
                return (
                  <div
                    key={v.id}
                    className={`p-2.5 rounded-xl bg-[#090e1a] border ${unitStyles.borderGlow} flex items-center justify-between text-xs`}
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <span className="text-[11px] text-slate-400">{v.driver} • {v.station}</span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      {onOpenRadioCall && (
                        <button
                          onClick={() => onOpenRadioCall({ type: 'Unit', id: v.id, name: v.name, vehicle: v })}
                          className="p-1.5 bg-emerald-500/15 hover:bg-emerald-500 text-emerald-400 hover:text-white rounded-lg border border-emerald-500/30 transition-colors"
                          title={`Radio Call ${v.id}`}
                        >
                          <PhoneCall size={12} />
                        </button>
                      ...
                      <div className="text-right">
                        <span className="text-[10px] text-emerald-400 font-mono block">98% Sig</span>
                        <span className="text-[10px] text-slate-400 font-mono">{v.fuel}% Fuel</span>
                      </div>
                    </div>
                  </div>
                );
          ...
          <div className={`mt-3.5 p-3 rounded-xl border flex items-center justify-between text-xs ${
            ...
            {onNavigateToMap && (
              <button
                ...
                className="px-2.5 py-1 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg text-xs font-bold shrink-0 ml-2 shadow-xs cursor-pointer"
              >
                ...
                >
                  ...
                  <div className={`pt-1 flex items-center justify-between gap-1.5 border-t ${
                    ...
                    <button
                      ...
                      className="px-2.5 py-1 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg text-[10px] font-bold shadow-xs transition-all disabled:opacity-50 cursor-pointer"
                    >
                      +50 Inflow
                    </button>
                    <button
                      ...
                      className="px-2.5 py-1 bg-amber-600 hover:bg-amber-500 text-white rounded-lg text-[10px] font-bold shadow-xs transition-all disabled:opacity-50 cursor-pointer"
                    >
                      -50 Discharged
                    </button>
                    {onRemoveSanctuary && (
            ...
            >
              ...
              <div className={`pt-3 border-t flex items-center justify-between gap-2 ${
                ...
                <button
                  ...
                  className="flex-1 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold rounded-xl transition-colors flex items-center justify-center gap-1.5 shadow-sm cursor-pointer"
                >
          ...
          <div className={`border rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-150 ${
            ...
            <form onSubmit={handleCreateSanctuarySubmit} className="p-5 space-y-4 max-h-[80vh] overflow-y-auto">
              ...
              <div className={`pt-3 border-t flex items-center justify-end gap-2 ${
                ...
                <button
                  type="submit"
                  className="px-5 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-bold shadow-md transition-all flex items-center gap-1.5 cursor-pointer"
                >
                  <Check size={14} />
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h2 className="text-lg font-bold text-slate-100">Emergency Response Departments & Stations</h2>
          <p className="text-xs text-slate-400">Coordinated command heads, police precincts, fire stations, and medical networks</p>
        </div>
        ...
        <div className="flex items-center gap-2">
          <div className="relative">
            ...
            <input
              ...
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              className="bg-[#0e1626] border border-[#1b2b46] pl-7 pr-3 py-1.5 rounded-xl text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:border-blue-500 w-44 sm:w-56"
            />
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5">
        {depts.map(d => {
          const Icon = d.icon;
          return (
            <div key={d.name} className="bg-[#0b101d] border border-[#172338] rounded-2xl p-4 shadow-sm hover:border-slate-700 transition-colors">
              <div className="flex items-center gap-3 mb-2.5">
                <div className={`p-2.5 rounded-xl border ${
                  d.color === 'red' ? 'bg-red-500/15 text-red-400 border-red-500/30' :
                  d.color === 'emerald' ? 'bg-emerald-500/15 text-emerald-400 border-emerald-500/30' :
                  d.color === 'cyan' ? 'bg-cyan-500/15 text-cyan-400 border-cyan-500/30' :
                  'bg-blue-500/15 text-blue-400 border-blue-500/30'
                }`}>
                  <Icon size={18} />
                </div>
                <div className="min-w-0">
                  <h3 className="font-bold text-xs text-slate-100 truncate">{d.name}</h3>
                  <p className="text-[11px] text-slate-400 truncate">{d.head}</p>
                </div>
              </div>
              <div className="pt-2 border-t border-[#172338] flex justify-between items-center text-[11px]">
                <span className="text-slate-400">
                  Fleet: <strong className="text-slate-200">{d.activeUnits} units</strong>
                </span>
                <span className="font-mono text-blue-400 font-bold">{d.hotline}</span>
              </div>
            </div>
          );
        })}
      ...
      <div className="flex bg-[#0e1626] p-1 rounded-xl border border-[#1b2b46] w-fit gap-1">
        <button
        ...
        <button
          ...
          className={`px-3 py-1.5 text-xs font-semibold rounded-lg transition-all flex items-center gap-1.5 ${
            activeTab === 'Police' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'
          }`}
        >
        ...
        <button
          ...
          className={`px-3 py-1.5 text-xs font-semibold rounded-lg transition-all flex items-center gap-1.5 ${
            activeTab === 'Fire' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'
          }`}
        >
        ...
        <button
          ...
          className={`px-3 py-1.5 text-xs font-semibold rounded-lg transition-all flex items-center gap-1.5 ${
            activeTab === 'Medical' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'
          }`}
        >
      ...
      {(activeTab === 'All' || activeTab === 'Police') && (
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-sm text-slate-100">Police Stations & Precincts ({filteredPolice.length})</h3>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3.5">
            {filteredPolice.map(p => (
              <div key={p.id} className="bg-[#0b101d] border border-[#172338] rounded-2xl p-4 shadow-sm hover:border-blue-500/40 transition-all flex flex-col justify-between">
                <div>
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <div className="text-[10px] font-mono text-blue-400 mb-0.5">{p.id} • {p.division} Division</div>
                      <h4 className="font-bold text-sm text-slate-100">{p.name}</h4>
                    </div>
                    <span className="px-2 py-0.5 rounded text-[10px] font-bold border text-blue-400 bg-blue-500/10 border-blue-500/30 shrink-0">
                      Precinct
                    </span>
                  </div>

                  <div className="mt-3 space-y-1.5 bg-[#090e1a] p-2.5 rounded-xl border border-[#16243a] text-xs">
                    <div className="flex justify-between items-center text-slate-300">
                      <span className="text-slate-400">Personnel:</span>
                      <strong className="text-slate-100 font-mono">{p.personnel} Officers</strong>
                    </div>
                    <div className="flex justify-between items-center text-slate-300">
                      <span className="text-slate-400">Patrol Vehicles:</span>
                      <strong className="text-blue-300 font-mono">{p.vehiclesCount || 8} Active Units</strong>
                    </div>
                    <div className="flex justify-between items-center text-slate-400 text-[10px] pt-1 border-t border-[#131d2e]">
                      <span>Coordinates:</span>
                ...
                <div className="pt-3 mt-3 border-t border-[#172338] flex items-center justify-between gap-2 text-xs">
                  {p.contact && (
                    <a
                      href={`tel:${p.contact}`}
                      className="px-2.5 py-1 bg-[#132035] hover:bg-[#1c3050] text-blue-300 rounded-lg font-bold border border-blue-500/30 flex items-center gap-1"
                    >
                  ...
                  <button
                    ...
                    className="px-2.5 py-1 bg-blue-600 hover:bg-blue-500 text-white rounded-lg font-bold transition-colors ml-auto flex items-center gap-1"
                  >
      ...
      {(activeTab === 'All' || activeTab === 'Fire') && (
        <div className="space-y-3 pt-2">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-sm text-slate-100">Fire & Rescue Stations ({filteredFire.length})</h3>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3.5">
            {filteredFire.map(f => (
              <div key={f.id} className="bg-[#0b101d] border border-[#172338] rounded-2xl p-4 shadow-sm hover:border-red-500/40 transition-all flex flex-col justify-between">
                <div>
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <div className="text-[10px] font-mono text-red-400 mb-0.5">{f.id} • {f.zone} Zone</div>
                      <h4 className="font-bold text-sm text-slate-100">{f.name}</h4>
                    </div>
                    <span className="px-2 py-0.5 rounded text-[10px] font-bold border text-red-400 bg-red-500/10 border-red-500/30 shrink-0">
                      Fire Base
                    </span>
                  </div>
                  ...
                  <div className="mt-3 space-y-1.5 bg-[#090e1a] p-2.5 rounded-xl border border-[#16243a] text-xs">
                    <div className="flex justify-between items-center text-slate-300">
                      <span className="text-slate-400">Engines / Vehicles:</span>
                      <strong className="text-red-400 font-mono">{f.vehiclesCount} Tender Engines</strong>
                    </div>
                    {f.personnel && (
                      <div className="flex justify-between items-center text-slate-300">
                        <span className="text-slate-400">Firefighters:</span>
                        <strong className="text-slate-100 font-mono">{f.personnel} Personnel</strong>
                      </div>
                    )}
                    <div className="flex justify-between items-center text-slate-400 text-[10px] pt-1 border-t border-[#131d2e]">
                      <span>Coordinates:</span>
                      <span className="font-mono text-slate-300">{f.lat.toFixed(4)}, {f.lng.toFixed(4)}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="pt-3 mt-3 border-t border-[#172338] flex items-center justify-between gap-2 text-xs">
                  {f.contact && (
                    <a
                      href={`tel:${f.contact}`}
                      className="px-2.5 py-1 bg-[#132035] hover:bg-[#1c3050] text-red-300 rounded-lg font-bold border border-red-500/30 flex items-center gap-1"
                    >
                  ...
                  <button
                    ...
                    className="px-2.5 py-1 bg-red-600 hover:bg-red-500 text-white rounded-lg font-bold transition-colors ml-auto flex items-center gap-1"
                  >
      ...
      {(activeTab === 'All' || activeTab === 'Medical') && (
        <div className="space-y-3 pt-2">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-sm text-slate-100">Receiving Trauma & Hospital Network ({filteredHospitals.length})</h3>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3.5">
            {filteredHospitals.map(h => (
              <div key={h.id} className="bg-[#0b101d] border border-[#172338] rounded-2xl p-4 shadow-sm hover:border-emerald-500/40 transition-all flex flex-col justify-between">
                <div>
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <div className="text-[10px] font-mono text-emerald-400 mb-0.5">{h.id} • {h.area || 'Metro'}</div>
                      <h4 className="font-bold text-sm text-slate-100">{h.name}</h4>
                    </div>
                    <span className={`px-2 py-0.5 rounded text-[10px] font-bold border shrink-0 ${
                      h.type === 'Government' ? 'text-emerald-400 bg-emerald-500/10 border-emerald-500/30' : 'text-cyan-400 bg-cyan-500/10 border-cyan-500/30'
                    }`}>
                      {h.type}
                    </span>
                  </div>
                  ...
                  <div className="mt-3 space-y-1.5 bg-[#090e1a] p-2.5 rounded-xl border border-[#16243a] text-xs">
                    ...
                    {h.availableBeds && (
                      <div className="flex justify-between items-center text-slate-300">
                        ...
                        <strong className="text-emerald-400 font-mono">{h.availableBeds} Free</strong>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="pt-3 mt-3 border-t border-[#172338] flex items-center justify-between gap-2 text-xs">
                  {h.contact && (
                    <a
                      href={`tel:${h.contact}`}
                      className="px-2.5 py-1 bg-[#132035] hover:bg-[#1c3050] text-emerald-400 rounded-lg font-bold border border-emerald-500/30 flex items-center gap-1"
                    >
                  ...
                  <button
                    ...
                    className="px-2.5 py-1 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg font-bold transition-colors ml-auto flex items-center gap-1"
                  >
  ...
  return (
    <div className="space-y-6 max-w-6xl">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-[#172338]">
        <div>
          <div className="flex items-center gap-2.5">
            <div className="p-2 rounded-xl bg-red-500/15 text-red-400 border border-red-500/30 shadow-[0_0_15px_rgba(239,68,68,0.2)]">
              <Megaphone size={20} />
            </div>
            <div>
              <h2 className="text-lg font-bold text-slate-100">Public Warning & Emergency Broadcasts</h2>
              <p className="text-xs text-slate-400">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <button
            ...
            className="px-4 py-2 bg-red-600 hover:bg-red-500 text-white rounded-xl text-xs font-bold transition-all shadow-[0_0_15px_rgba(239,68,68,0.35)] flex items-center gap-1.5"
          >
            <Plus size={14} />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div
          ...
          className="bg-[#0b101d] border border-red-500/30 hover:border-red-500/60 p-3.5 rounded-2xl cursor-pointer transition-all hover:bg-[#10182a]"
        >
          ...
          <h4 className="font-bold text-xs text-slate-100 mt-1">Hazardous / Structural Evacuation</h4>
          <p className="text-[11px] text-slate-400 mt-0.5">Dispatches siren towers and emergency cell broadcast</p>
        </div>
        ...
        <div
          ...
          className="bg-[#0b101d] border border-amber-500/30 hover:border-amber-500/60 p-3.5 rounded-2xl cursor-pointer transition-all hover:bg-[#10182a]"
        >
          ...
          <h4 className="font-bold text-xs text-slate-100 mt-1">Arterial Highway Road Closure</h4>
          <p className="text-[11px] text-slate-400 mt-0.5">Updates dynamic highway VMS boards & traffic radio</p>
        </div>
        ...
        <div
          ...
          className="bg-[#0b101d] border border-emerald-500/30 hover:border-emerald-500/60 p-3.5 rounded-2xl cursor-pointer transition-all hover:bg-[#10182a]"
        >
          ...
          <h4 className="font-bold text-xs text-slate-100 mt-1">Green Corridor Traffic Priority</h4>
          <p className="text-[11px] text-slate-400 mt-0.5">Alerts incoming traffic and smart signals</p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="font-bold text-sm text-slate-200">Active City Broadcasts ({broadcasts.length})</h3>
          <span className="text-xs text-slate-400 font-mono">Real-time CAP Public Feed</span>
        </div>
        ...
        <div className="space-y-3.5">
          {broadcasts.map(bc => (
            <div
              key={bc.id}
              className={`bg-[#0b101d] border rounded-2xl p-4 sm:p-5 shadow-lg transition-all ${
                bc.severity === 'Extreme'
            ...
            >
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-[#172338] pb-3">
                <div className="flex items-center gap-2.5 flex-wrap">
                  <span className="font-mono text-xs font-bold text-slate-400">{bc.id}</span>
                  <h4 className="font-bold text-sm text-slate-100">{bc.title}</h4>
                  <span
                ...
                <div className="flex items-center gap-3 text-xs text-slate-400 font-mono">
                  ...
                  <span className="text-emerald-400 font-bold">{bc.reach}</span>
                </div>
              </div>
              ...
              <div className="mt-3.5 pt-3 border-t border-[#172338] flex flex-wrap items-center justify-between gap-3 text-xs">
                <div className="flex items-center gap-1.5 flex-wrap">
                  <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Broadcasted Via:</span>
                  {bc.channels.map(ch => (
                    <span key={ch} className="px-2 py-0.5 bg-[#090e1a] text-slate-300 rounded-md border border-[#1a2942] text-[10px] font-medium">
                      {ch}
                    </span>
                  ))}
                </div>
                ...
                <div className="flex items-center gap-2 text-slate-400 text-[11px]">
                  <span>Zone: <strong className="text-slate-200">{bc.zone}</strong></span>
                  <span className="text-slate-600">•</span>
                  <span>Authorizer: <strong className="text-slate-200">{bc.author}</strong></span>
                </div>
              </div>
            </div>
          ))}
      ...
      {showNewModal && (
        <div className="fixed inset-0 z-[2000] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#0b101d] border border-[#223554] rounded-2xl w-full max-w-xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-4 border-b border-[#172338] flex justify-between items-center bg-[#070b14]">
              <div className="flex items-center gap-2">
                <Megaphone size={16} className="text-red-400" />
                <h3 className="font-bold text-sm text-slate-100">Issue Emergency Public Broadcast (CAP)</h3>
              </div>
              <button
            ...
            <form onSubmit={handlePublish} className="p-5 space-y-4 text-xs">
              <div>
                <label className="block text-slate-300 font-bold mb-1">Broadcast Title / Headline</label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full bg-[#080d18] border border-[#1b2b46] rounded-xl px-3 py-2 text-slate-200 focus:outline-none focus:border-red-500 font-medium"
                />
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-300 font-bold mb-1">Target Geographic Zone</label>
                  <select
                    value={newZone}
                    onChange={(e) => setNewZone(e.target.value)}
                    className="w-full bg-[#080d18] border border-[#1b2b46] rounded-xl px-3 py-2 text-slate-200 focus:outline-none focus:border-blue-500"
                  >
                ...
                <div>
                  <label className="block text-slate-300 font-bold mb-1">Severity / Urgency Level</label>
                  <select
                    value={newSeverity}
                    onChange={(e) => setNewSeverity(e.target.value as any)}
                    className="w-full bg-[#080d18] border border-[#1b2b46] rounded-xl px-3 py-2 text-slate-200 focus:outline-none focus:border-red-500 font-bold"
                  >
              ...
              <div>
                <label className="block text-slate-300 font-bold mb-1">Public Broadcast Message Body</label>
                <textarea
                  required
                  rows={4}
                  ...
                  className="w-full bg-[#080d18] border border-[#1b2b46] rounded-xl p-3 text-slate-200 focus:outline-none focus:border-blue-500 leading-relaxed"
                />
              </div>

              <div>
                <label className="block text-slate-300 font-bold mb-2">Dispatched Public Channels</label>
                      ...
                      <button
                        ...
                        className={`p-2 rounded-xl border text-left text-[11px] font-medium transition-all ${
                          isSelected
                            ...
                            : 'bg-[#080d18] border-[#1b2b46] text-slate-400 hover:text-slate-200'
                        }`}
                      >
                        {isSelected ? '✓ ' : '+ '} {ch}
                      </button>
              ...
              <div className="pt-3 border-t border-[#172338] flex justify-end gap-2">
                <button
                  type="button"
                  onClick={() => setShowNewModal(false)}
                  className="px-4 py-2 bg-[#15233c] hover:bg-[#1e3254] text-slate-300 rounded-xl font-bold transition-colors"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="px-5 py-2 bg-red-600 hover:bg-red-500 text-white rounded-xl font-bold transition-all shadow-[0_0_15px_rgba(239,68,68,0.4)] flex items-center gap-1.5"
                >
                  <Send size={13} />
  ...
  return (
    <div className="space-y-5 max-w-5xl">
        ...
        <div className="flex items-center gap-2.5">
          {unreadCount > 0 && (
            <button
              ...
              className="px-3 py-1.5 bg-blue-600/20 hover:bg-blue-600/30 text-blue-300 border border-blue-500/40 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5"
            >
          ...
          {notifications.length > 0 && (
            <button
              onClick={onClearAll}
              className="px-3 py-1.5 bg-[#15233c] hover:bg-[#1f3458] text-slate-300 hover:text-white border border-[#223554] rounded-xl text-xs font-bold transition-all flex items-center gap-1.5"
            >
              <Trash2 size={13} />
              <span>Clear Log</span>
            </button>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="bg-[#0b101d] border border-[#172338] rounded-2xl p-3.5 shadow-sm">
          <span className="text-[10px] uppercase font-bold text-slate-400 block">Total Alerts Logged</span>
          <span className="text-xl font-bold font-mono text-slate-100 mt-0.5 block">{notifications.length}</span>
          <span className="text-[10px] text-slate-500">Continuous operation log</span>
        </div>

        <div className="bg-[#0b101d] border border-[#172338] rounded-2xl p-3.5 shadow-sm">
          <span className="text-[10px] uppercase font-bold text-slate-400 block">Unread Notifications</span>
          <span className={`text-xl font-bold font-mono mt-0.5 block ${unreadCount > 0 ? 'text-red-400' : 'text-slate-400'}`}>
            {unreadCount}
          </span>
          <span className="text-[10px] text-slate-500">Requires dispatcher check</span>
        </div>

        <div className="bg-[#0b101d] border border-[#172338] rounded-2xl p-3.5 shadow-sm">
          <span className="text-[10px] uppercase font-bold text-slate-400 block">Resolved Incidents</span>
          <span className="text-xl font-bold font-mono text-emerald-400 mt-0.5 block">{resolutionCount}</span>
          <span className="text-[10px] text-emerald-500/80">Archived & secured</span>
        </div>

        <div className="bg-[#0b101d] border border-[#172338] rounded-2xl p-3.5 shadow-sm">
          <span className="text-[10px] uppercase font-bold text-slate-400 block">Active Dispatch Updates</span>
          <span className="text-xl font-bold font-mono text-blue-400 mt-0.5 block">{dispatchCount}</span>
          <span className="text-[10px] text-slate-500">Fleet telemetry events</span>
        </div>
      </div>

      {/* Filter and Search Toolbar */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-[#0b101d] border border-[#172338] p-3 rounded-2xl">
        ...
        <div className="relative w-full sm:w-60">
          ...
          <input
            ...
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            className="w-full bg-[#0e1626] border border-[#1b2b46] pl-8 pr-3 py-1.5 rounded-xl text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:border-blue-500"
          />
        </div>
      </div>

      {/* Notification List (Solid, Permanent & Readable) */}
      <div className="space-y-3">
        {filtered.length === 0 ? (
          <div className="bg-[#0b101d] border border-[#172338] rounded-2xl p-10 text-center space-y-2">
            <BellRing size={32} className="mx-auto text-slate-600 mb-2" />
            <h3 className="text-sm font-bold text-slate-300">No Notifications in this Category</h3>
          ...
          filtered.map(item => {
            ...
            let iconBox = 'bg-blue-500/15 text-blue-400 border-blue-500/30';
            ...
            if (item.type === 'Resolution') {
              ...
              typeBadge = 'bg-emerald-500/10 text-emerald-300 border-emerald-500/40';
            } else if (item.type === 'Incident') {
              iconBox = 'bg-red-500/15 text-red-400 border-red-500/30';
              IconComponent = AlertTriangle;
              typeBadge = 'bg-red-500/10 text-red-300 border-red-500/40';
            } else if (item.type === 'Dispatch') {
              iconBox = 'bg-cyan-500/15 text-cyan-400 border-cyan-500/30';
              IconComponent = Car;
              typeBadge = 'bg-cyan-500/10 text-cyan-300 border-cyan-500/40';
            } else if (item.type === 'Arrival') {
              iconBox = 'bg-amber-500/15 text-amber-400 border-amber-500/30';
              IconComponent = MapPin;
              typeBadge = 'bg-amber-500/10 text-amber-300 border-amber-500/40';
            } else if (item.type === 'Broadcast') {
              iconBox = 'bg-purple-500/15 text-purple-400 border-purple-500/30';
              IconComponent = Megaphone;
              typeBadge = 'bg-purple-500/10 text-purple-300 border-purple-500/40';
            }

            return (
              <div
                key={item.id}
                className={`bg-[#0b101d] border rounded-2xl p-4 sm:p-4.5 flex flex-col sm:flex-row gap-3.5 sm:items-start transition-all shadow-sm ${
                  !item.isRead
                    ? 'border-blue-500/50 bg-[#0d1527]/90 shadow-[0_0_15px_rgba(59,130,246,0.1)]'
                    : 'border-[#172338] hover:border-slate-700'
                }`}
              >
                {/* Left Type Icon Box */}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <h4 className={`font-bold text-sm ${!item.isRead ? 'text-white font-black' : 'text-slate-200'}`}>
                        {item.title}
                      </h4>
                      {!item.isRead && (
                  ...
                  <div className="mt-3 pt-2.5 border-t border-[#172338] flex flex-wrap items-center justify-between gap-2 text-xs">
                    <div className="flex items-center gap-2 flex-wrap">
                      {item.incidentId && (
                        <span className="font-mono text-[11px] text-slate-400 bg-[#080e1a] px-2 py-0.5 rounded border border-[#1a2942]">
                          Ref: <strong className="text-slate-200">{item.incidentId}</strong>
                        </span>
                      )}

                      {item.unitId && (
                        <span className="font-mono text-[11px] text-slate-400 bg-[#080e1a] px-2 py-0.5 rounded border border-[#1a2942]">
                          Unit: <strong className="text-slate-200">{item.unitId}</strong>
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex items-center gap-2 ml-auto">
                      ...
                      <button
                        ...
                        className="px-2.5 py-1 bg-[#132035] hover:bg-[#1b2f4f] text-slate-300 rounded-lg text-xs font-medium border border-[#1e3252] transition-colors"
...
export const VideoConferenceView: React.FC = () => {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-lg font-bold text-slate-100">Live CCTV & Video Surveillance Grid</h2>
        <p className="text-xs text-slate-400">Real-time tactical junction camera feeds</p>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {cameras.map(c => (
          <div
            key={c.name}
            className={`bg-[#0b101d] border ${c.color} rounded-2xl overflow-hidden shadow-lg flex flex-col`}
          >
            <div className="h-44 bg-[#070a12] relative flex items-center justify-center">
            ...
            <div className="p-3.5 bg-[#0a0f1d] border-t border-[#172338] flex justify-between items-center text-xs">
...
export const ReportsAnalyticsView: React.FC = () => (
  <div className="space-y-5">
    <div className="flex justify-between items-center">
      <div>
        <h2 className="text-lg font-bold text-slate-100">Reports & Operational Analytics</h2>
        <p className="text-xs text-slate-400">Response time analytics and incident metrics</p>
      </div>
    </div>
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      <div className="bg-[#0b101d] border border-[#172338] rounded-2xl p-5 shadow-sm">
        <h3 className="font-bold text-sm text-slate-200 mb-4 flex items-center gap-2">
          <BarChart3 size={16} className="text-blue-400" /> Incidents by Category
        </h3>
        <div className="space-y-3.5 text-xs">
          <div>
            <div className="flex justify-between mb-1">
              <span className="text-slate-300 font-semibold">Fire Emergencies</span>
              <span className="font-mono text-red-400 font-bold">45%</span>
            </div>
          ...
          <div>
            <div className="flex justify-between mb-1">
              <span className="text-slate-300 font-semibold">Medical Emergencies</span>
          ...
          <div>
            <div className="flex justify-between mb-1">
              <span className="text-slate-300 font-semibold">Traffic Collisions</span>
      ...
      <div className="bg-[#0b101d] border border-[#172338] rounded-2xl p-5 shadow-sm">
        <h3 className="font-bold text-sm text-slate-200 mb-4 flex items-center gap-2">
          <Clock size={16} className="text-amber-400" /> Average Response Times
        </h3>
        <div className="flex items-end justify-around h-36 border-b border-[#172338] pb-2">
          <div className="flex flex-col items-center gap-1.5">
            <span className="text-[11px] font-mono text-red-400 font-bold">4.2m</span>
            <div className="w-10 bg-red-500 rounded-t-lg h-24"></div>
            <span className="text-[11px] font-semibold text-slate-400">High</span>
          </div>
          <div className="flex flex-col items-center gap-1.5">
            <span className="text-[11px] font-mono text-amber-400 font-bold">8.5m</span>
            <div className="w-10 bg-amber-500 rounded-t-lg h-16"></div>
            <span className="text-[11px] font-semibold text-slate-400">Medium</span>
          </div>
          <div className="flex flex-col items-center gap-1.5">
            <span className="text-[11px] font-mono text-blue-400 font-bold">14.1m</span>
...
export const UsersView: React.FC = () => (
  <div className="space-y-5">
    <div className="flex justify-between items-center">
      <div>
        <h2 className="text-lg font-bold text-slate-100">User & Personnel Management</h2>
        <p className="text-xs text-slate-400">Dispatch officers and field response personnel</p>
      </div>
    </div>
      ...
      <table className="w-full text-left text-xs whitespace-nowrap">
        ...
        <tbody className="divide-y divide-[#151f32] text-slate-300">
          {[
            ...
            <tr key={u.name} className="hover:bg-[#111927]/60">
              <td className="px-5 py-3.5 font-bold text-slate-200">{u.name}</td>
              <td className="px-5 py-3.5 text-slate-400">{u.role}</td>
              <td className="px-4 py-3.5">
...
export const SettingsView: React.FC = () => {
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        <h2 className="text-lg font-bold text-slate-100">System Settings & Visual Appearance</h2>
        <p className="text-xs text-slate-400">Configure command console theme, dispatch automation, and notification preferences</p>
      </div>

      {/* Theme Selector Section */}
      <div className="bg-[#0b101d] border border-[#172338] rounded-2xl p-5 space-y-4 shadow-sm">
        <div className="flex items-center gap-2.5 pb-3 border-b border-[#172338]">
          <Palette size={18} className="text-blue-400" />
          <div>
            <h3 className="text-sm font-bold text-slate-100">Display Theme & Operational Mode</h3>
            <p className="text-[11px] text-slate-400">Select the display mode optimized for your operating environment</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 pt-1">
          ...
          <div
            ...
            className={`relative p-4 rounded-xl border-2 cursor-pointer transition-all ${
              isDark
                ...
                : 'bg-[#070b14] border-[#1d2a42] hover:border-slate-600 opacity-80 hover:opacity-100'
            }`}
          >
            ...
            <div className="space-y-1 mb-3">
              <h4 className="text-xs font-bold text-slate-100">Tactical Dark Mode (Night Ops)</h4>
          ...
          <div
            ...
            className={`relative p-4 rounded-xl border-2 cursor-pointer transition-all ${
              isLight
                ? 'bg-slate-100 border-blue-500 shadow-[0_0_16px_rgba(59,130,246,0.2)]'
                : 'bg-[#070b14] border-[#1d2a42] hover:border-slate-600 opacity-80 hover:opacity-100'
            }`}
          >
            <div className="flex items-start justify-between mb-3">
              ...
              {isLight ? (
                <span className="px-2 py-0.5 rounded-md bg-blue-500/20 border border-blue-500/40 text-blue-600 text-[10px] font-bold font-mono flex items-center gap-1">
                  <Check size={10} /> ACTIVE
                </span>
              ) : (
                <span className="text-[10px] text-slate-500 font-medium">Select</span>
            ...
            <div className="space-y-1 mb-3">
              <h4 className="text-xs font-bold text-slate-100">Tactical Light Mode (Daytime Ops)</h4>
              <p className="text-[11px] text-slate-400 leading-relaxed">
      ...
      <div className="bg-[#0b101d] border border-[#172338] rounded-2xl p-5 space-y-4 shadow-sm text-xs">
        <h3 className="text-sm font-bold text-slate-100 pb-2 border-b border-[#172338]">Automation & Alert Configuration</h3>
        
        <div className="flex justify-between items-center py-2 border-b border-[#172338]">
          <div>
            <p className="font-bold text-slate-200">Auto-Dispatch Proximity Algorithm</p>
            <p className="text-slate-400 text-[11px]">Automatically routes the closest available vehicle to high-priority events</p>
          </div>
          <div 
            onClick={() => setAutoDispatch(!autoDispatch)}
            className={`w-9 h-5 rounded-full relative cursor-pointer transition-colors ${autoDispatch ? 'bg-blue-600' : 'bg-slate-700'}`}
          >
        ...
        <div className="flex justify-between items-center py-2 border-b border-[#172338]">
          <div>
            <p className="font-bold text-slate-200">Audio Siren Alarms for High Priority</p>
            <p className="text-slate-400 text-[11px]">Triggers auditory sirens and voice warnings on critical incoming alerts</p>
          </div>
          <div 
            onClick={() => setAudioSirens(!audioSirens)}
            className={`w-9 h-5 rounded-full relative cursor-pointer transition-colors ${audioSirens ? 'bg-blue-600' : 'bg-slate-700'}`}
          >
        ...
        <div className="flex justify-between items-center py-2 border-b border-[#172338]">
          <div>
            <p className="font-bold text-slate-200">Tactical Radio Sound FX & Chirps</p>
            <p className="text-slate-400 text-[11px]">Play Push-to-Talk squeaks and radio static when receiving voice transmissions</p>
          </div>
          <div 
            onClick={() => setRadioSounds(!radioSounds)}
            className={`w-9 h-5 rounded-full relative cursor-pointer transition-colors ${radioSounds ? 'bg-blue-600' : 'bg-slate-700'}`}
          >
        ...
        <div className="flex justify-between items-center py-2">
          <div>
            <p className="font-bold text-slate-200">Live Green Emergency Corridors</p>
            <p className="text-slate-400 text-[11px]">Auto-calculate traffic preemption and fastest hospital transit routes</p>
          </div>
          <div 
            onClick={() => setTacticalCorridors(!tacticalCorridors)}
            className={`w-9 h-5 rounded-full relative cursor-pointer transition-colors ${tacticalCorridors ? 'bg-blue-600' : 'bg-slate-700'}`}
          >
