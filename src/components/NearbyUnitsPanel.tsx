  ...
  return (
    <div className={`flex flex-col overflow-hidden ${
      compact
        ...
        : isDark
        ? 'bg-[#0b101d] border border-[#172338] rounded-2xl p-4 sm:p-5 shadow-lg'
      ...
      <div className={`flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b ${
        ...
        <div className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border ${
          isDark ? 'bg-[#080d18] border-[#1a2942]' : 'bg-slate-50 border-slate-200'
        }`}>
          ...
          >
            {incidents.filter(i => i.status !== 'Resolved' && i.status !== 'Closed').map(inc => (
              <option key={inc.id} value={inc.id} className={isDark ? 'bg-[#0b101d] text-slate-200' : 'bg-white text-slate-800'}>
      ...
      <div className={`grid grid-cols-3 gap-2 py-2.5 border-b text-center ${
        isDark ? 'border-[#172338]/80' : 'border-slate-200'
      }`}>
        <div className={`py-1.5 px-2 rounded-xl border ${
          isDark ? 'bg-[#080e1a] border-[#142033]' : 'bg-emerald-50/60 border-emerald-200'
        }`}>
          <span className={`text-[10px] font-medium block ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>Available</span>
          <span className="text-xs font-bold font-mono text-emerald-600 dark:text-emerald-400">{availableCount} Units</span>
        </div>
        <div className={`py-1.5 px-2 rounded-xl border ${
          isDark ? 'bg-[#080e1a] border-[#142033]' : 'bg-slate-50 border-slate-200'
        }`}>
          <span className={`text-[10px] font-medium block ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>En Route</span>
          <span className="text-xs font-bold font-mono text-emerald-700 dark:text-cyan-400">{dispatchedCount} Units</span>
        </div>
        <div className={`py-1.5 px-2 rounded-xl border ${
          isDark ? 'bg-[#080e1a] border-[#142033]' : 'bg-amber-50/60 border-amber-200'
        }`}>
      ...
      <div className={`flex flex-wrap items-center justify-between gap-2 py-2.5 border-b ${
        isDark ? 'border-[#172338]/70' : 'border-slate-200'
      }`}>
        <div className={`flex p-1 rounded-xl border gap-1 overflow-x-auto ${
          isDark ? 'bg-[#0e1626] border-[#1a2942]' : 'bg-slate-100 border-slate-200'
        }`}>
                  ...
                  className={`px-2.5 py-1 text-xs font-semibold rounded-lg transition-all flex items-center gap-1.5 whitespace-nowrap border cursor-pointer ${
                    isSelected
                      ? isDark
                        ...
                        : 'bg-white text-red-700 border border-red-500 shadow-xs font-bold'
                      : isDark
                      ? 'text-red-400 hover:bg-red-950/40 hover:text-red-200 border-red-500/20'
                      : 'text-slate-800 hover:bg-red-50 hover:text-red-700 border-transparent'
                  }`}
                >
                  <Flame size={12} className={isSelected ? (isDark ? 'text-white' : 'text-red-600') : 'text-red-500'} />
                  <span>Fire</span>
                </button>
              );
            }
                  ...
                  className={`px-2.5 py-1 text-xs font-semibold rounded-lg transition-all flex items-center gap-1.5 whitespace-nowrap border cursor-pointer ${
                    isSelected
                      ? isDark
                        ...
                        : 'bg-white text-blue-700 border border-blue-500 shadow-xs font-bold'
                      : isDark
                      ? 'text-cyan-400 hover:bg-cyan-950/40 hover:text-cyan-200 border-cyan-500/20'
                      : 'text-slate-800 hover:bg-slate-200 hover:text-slate-900 border-transparent'
                  }`}
                >
                  <Shield size={12} className={isSelected ? (isDark ? 'text-white' : 'text-blue-600') : 'text-blue-500'} />
                  <span>Police</span>
                </button>
              );
            }
                  ...
                  className={`px-2.5 py-1 text-xs font-semibold rounded-lg transition-all flex items-center gap-1.5 whitespace-nowrap border cursor-pointer ${
                    isSelected
                      ? isDark
                        ...
                        : 'bg-white text-emerald-700 border border-emerald-500 shadow-xs font-bold'
                      : isDark
                      ? 'text-emerald-400 hover:bg-emerald-950/40 hover:text-emerald-200 border-emerald-500/20'
                      : 'text-emerald-800 hover:bg-emerald-50 hover:text-emerald-900 border-transparent'
                  }`}
                >
                  <Stethoscope size={12} className={isSelected ? (isDark ? 'text-white' : 'text-emerald-600') : 'text-emerald-600'} />
                  <span>Ambulance</span>
                </button>
              );
            }
            return (
              <button
                ...
                className={`px-2.5 py-1 text-xs font-semibold rounded-lg transition-all whitespace-nowrap cursor-pointer border ${
                  isSelected
                    ? isDark
                      ? 'bg-blue-600 text-white shadow-sm font-bold border-blue-600'
                      : 'bg-white text-slate-900 border-slate-400 shadow-xs font-bold'
                    : isDark
                    ? 'text-slate-400 hover:text-slate-200 hover:bg-slate-800 border-transparent'
                    : 'text-slate-800 hover:text-slate-900 hover:bg-slate-200 border-transparent font-semibold'
                }`}
              >
        ...
        <div className="flex items-center gap-2.5">
          <button
            ...
            className={`flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold rounded-lg border transition-all cursor-pointer select-none ${
              onlyAvailable
                ...
                : isDark
                ? 'bg-[#111927] text-slate-400 border-slate-700/80 hover:text-slate-200'
          ...
          {closestUnit && closestUnit.status === 'Available' && (
            <button
              ...
              className={`hidden sm:inline-flex items-center gap-1 text-[11px] font-bold px-2 py-0.5 rounded-lg border transition-colors cursor-pointer ${
                isDark
                  ? 'bg-emerald-500/15 text-emerald-300 border-emerald-500/30 hover:bg-emerald-500/25'
                  : 'bg-emerald-50 text-emerald-800 border-emerald-300 hover:bg-emerald-100'
              }`}
            >
              <Zap size={11} className="text-emerald-600 dark:text-emerald-400" /> Dispatch Required Units
            </button>
          )}
        </div>
      </div>
      ...
      <div className="space-y-2 overflow-y-auto mt-2.5 flex-1 max-h-[460px] pr-0.5">
        {sortedVehicles.length === 0 ? (
          <div className={`p-6 text-center text-xs rounded-xl border ${
            isDark ? 'text-slate-400 bg-[#0e1626]/40 border-[#172338]' : 'text-slate-500 bg-slate-50 border-slate-200'
          }`}>
            ...
            return (
              <div
                ...
                className={`p-3 rounded-xl border transition-all duration-200 ${
                  vehicle.isAssignedToThis
                    ? isDark
                      ? 'bg-[#0f1d33] border-emerald-500/50 shadow-sm'
                      : 'bg-emerald-50/50 border-emerald-300 shadow-xs'
                    : vehicle.status === 'Available'
                    ? isDark
                      ? `bg-[#0e1626] ${unitStyles.borderGlow} hover:border-slate-500`
                      : `bg-white border-slate-200 hover:border-emerald-300 shadow-xs`
                    : isDark
                    ? 'bg-[#0a0f1d] border-[#141e30] opacity-80'
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
                  ...
                  <div className="flex items-start gap-2.5 min-w-0">
                    ...
                    <div className={`flex flex-col items-center justify-center w-7 h-7 rounded-lg font-mono font-bold text-[11px] shrink-0 border ${
                      isDark ? 'bg-[#15233c] text-emerald-400 border-emerald-500/20' : 'bg-slate-100 text-slate-700 border-slate-200'
                    }`}>
                      #{idx + 1}
                    </div>
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        <span className={`px-2 py-0.5 text-xs rounded-md border flex items-center gap-1 ${unitStyles.idBadge}`}>
                          {unitStyles.category === 'Fire' && <Flame size={12} className={isDark ? "text-red-300" : "text-red-600"} />}
                          {unitStyles.category === 'Police' && <Shield size={12} className={isDark ? "text-cyan-300" : "text-blue-600"} />}
                        ...
                        <span
                          className={`px-1.5 py-0.2 text-[10px] font-semibold rounded-md border ${
                            vehicle.status === 'Available'
                              ...
                              : vehicle.isAssignedToThis
                              ? isDark ? 'text-emerald-300 bg-emerald-500/20 border-emerald-500/40 animate-pulse' : 'text-emerald-800 bg-emerald-100 border-emerald-300 animate-pulse'
                      ...
                      <div className={`flex items-center gap-2.5 text-[11px] mt-1 flex-wrap ${
                        ...
                        <span className="flex items-center gap-1">
                          <Fuel size={11} className={isDark ? 'text-slate-500' : 'text-slate-400'} />
                ...
                <div className={`flex items-center justify-between sm:justify-end gap-3 shrink-0 pt-2 sm:pt-0 border-t sm:border-t-0 ${
                  isDark ? 'border-[#1a2942]' : 'border-slate-200'
                }`}>
                  {/* Distance & ETA Badge */}
                  <div className="text-left sm:text-right">
                    <div className="flex items-center sm:justify-end gap-1 text-xs font-mono font-bold text-amber-500">
                      <Navigation size={11} className="text-amber-500 rotate-45" />
                  ...
                  <div className="flex items-center gap-1.5">
                    {onOpenRadioCall && (
                      ...
                      >
                        <PhoneCall size={13} className="text-emerald-600 dark:text-emerald-400" />
                      </button>
                    )}
                    ...
                    {onOpenCommsWithVehicle && (
                      <button
                        ...
                        className={`p-1.5 rounded-xl border transition-colors cursor-pointer ${
                          isDark
                            ? 'bg-[#15233c] hover:bg-[#1f3458] text-slate-300 border-slate-700'
                    ...
                    {vehicle.isAssignedToThis ? (
                      <button
                        ...
                        className={`px-2.5 py-1 border rounded-xl text-xs font-bold transition-all flex items-center gap-1 shadow-xs cursor-pointer ${
                          isDark ? 'bg-red-600/20 hover:bg-red-600/30 text-red-400 border-red-500/30' : 'bg-red-50 hover:bg-red-100 text-red-700 border-red-300'
                        }`}
                      ...
                      <button
                        ...
                        className={`px-3 py-1 text-xs font-bold rounded-xl transition-all flex items-center gap-1 cursor-pointer shadow-xs ${
                          vehicle.status === 'Available'
                            ? 'bg-emerald-600 hover:bg-emerald-500 text-white'
                            : isDark
                            ? 'bg-[#15233c] hover:bg-[#1c3055] text-slate-200 border border-slate-700'
