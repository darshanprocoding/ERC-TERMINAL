...
export const DEFAULT_CHANNELS: ChatChannel[] = [
  {
    id: 'inc-015',
    name: '🚨 [INC-015] Kilpauk Fire Ops',
    category: 'Incident-Tactical',
    incidentId: 'INC-2025-015',
    unitIds: ['FE-12', 'PV-23'],
    description: 'Tactical comms for Kilpauk structural fire response',
    lastMessage: 'PV-23: North perimeter secured. Hydrant accessible.',
    lastTime: '10:41 AM',
    unreadCount: 0
  },
  {
    id: 'unit-amb07-pv23',
    name: '🚑 AMB-07 ⟷ 🚔 PV-23 Direct',
    category: 'Inter-Unit',
    unitIds: ['AMB-07', 'PV-23'],
    description: 'Direct Green Corridor & Traffic Escort link',
    lastMessage: 'AMB-07: Requesting green light corridor at Poonamallee junction.',
    lastTime: '10:40 AM',
    unreadCount: 1
  },
  {
    id: 'unit-fe12-pv23',
    name: '🚒 FE-12 ⟷ 🚔 PV-23 Direct',
    category: 'Inter-Unit',
    unitIds: ['FE-12', 'PV-23'],
    description: 'Fire command and police perimeter coordination',
    lastMessage: 'FE-12: Need civilian traffic pushed back 100 meters.',
    lastTime: '10:37 AM',
    unreadCount: 0
  },
  {
    ...
    lastMessage: 'Control: All units near Kilpauk, proceed to incident location.',
    lastTime: '10:35 AM',
    unreadCount: 0
  },
  {
...
export const INITIAL_MESSAGES: ChatMessage[] = [
  {
    ...
    senderType: 'Police',
    time: '10:38 AM',
    text: 'PV-23 on scene. Establishing north perimeter cordon. Diverting arterial traffic.',
    tacticalStatus: 'ON_SCENE'
  },
  {
    id: 'm2',
    channelId: 'inc-015',
    ...
    senderType: 'Fire',
    time: '10:40 AM',
    text: 'Fire unit FE-12 deployed. Connecting to hydrant. 2 hose lines charged.',
    tacticalStatus: 'ON_SCENE'
  },
  {
    id: 'm3',
    ...
    time: '10:41 AM',
    text: 'Control copy. Additional water tender dispatched from Anna Nagar base.'
  },
  {
    id: 'm4',
    channelId: 'unit-amb07-pv23',
    sender: 'Ambulance AMB-07',
    senderId: 'AMB-07',
    senderType: 'Medical',
    time: '10:39 AM',
    text: 'PV-23, we have high-acuity patient. Requesting police escort through Anna Salai.',
    isVoiceNote: true,
    voiceDuration: '0:06'
  },
  {
    ...
    senderType: 'Police',
    time: '10:40 AM',
    text: 'Copy AMB-07. PV-23 moving ahead of you at Sterling Road. Green light corridor open.',
    tacticalStatus: 'EN_ROUTE'
  },
  {
    id: 'm6',
...
export const CommunicationCenter: React.FC<CommunicationCenterProps> = ({
  ...
  const [internalActiveChannelId, setInternalActiveChannelId] = useState<string>('inc-015');
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className={`flex items-center gap-1 text-[9px] font-mono px-1.5 py-0.2 rounded border ${
                isDark ? 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20' : 'text-emerald-700 bg-emerald-50 border-emerald-300 font-semibold'
              }`}>
                <Lock size={9} /> AES-256
              </span>
            </div>
        ...
        <div className="flex items-center gap-1.5">
          {micState !== 'granted' && (
            <button
              ...
              className={`p-1.5 rounded-lg border text-xs transition-all flex items-center gap-1 animate-pulse ${
                isDark ? 'bg-emerald-600/20 hover:bg-emerald-600/30 text-emerald-300 border-emerald-500/40' : 'bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border-emerald-300'
              }`}
              title="Allow Microphone Access for Tactical Radio"
            >
              <Mic size={13} className={isDark ? "text-emerald-400" : "text-emerald-600"} />
          ...
          <button
            ...
            className={`p-1.5 rounded-lg border text-xs transition-all flex items-center gap-1 shadow-sm ${
              isDark 
          ...
          >
            <PhoneCall size={13} className={isDark ? "text-emerald-400" : "text-white"} />
          ...
          <button
            ...
            className={`p-1.5 rounded-lg border text-xs transition-colors flex items-center gap-1 ${
              isDark 
                ? 'bg-[#15233c] hover:bg-[#1c3258] text-emerald-400 border-emerald-500/30' 
                : 'bg-slate-100 hover:bg-slate-200 text-slate-700 border-slate-300'
            }`}
            title="Create Direct Unit Link"
          >
            <Plus size={13} />
            <span className="text-[11px] font-bold hidden sm:inline">Direct Link</span>
          </button>
        ...
        <>
              ...
              return (
                <button
                  ...
                  className={`px-2.5 py-1.5 rounded-xl text-xs font-semibold shrink-0 transition-all flex items-center gap-1.5 border ${
                    isSelected
                      ? isDark
                        ? 'bg-[#15233c] text-emerald-300 border-emerald-500/40 shadow-sm'
                        : 'bg-emerald-600 text-white border-emerald-600 shadow-sm font-bold'
                      : isDark
                      ? 'text-slate-400 hover:text-slate-200 bg-[#0c1220] border-[#18253b]'
          ...
          {activeChannel && (
            <div className={`px-3 py-1.5 border-b flex items-center justify-between text-[11px] ${
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                <span className={`font-mono text-[10px] font-bold ${isDark ? 'text-emerald-400' : 'text-emerald-700'}`}>CH-NET OPEN</span>
              </div>
            </div>
          )}
                ...
                <div
                  ...
                  className={`p-2.5 rounded-xl text-xs space-y-1.5 border transition-all ${
                    msg.tacticalStatus === 'BACKUP_REQUESTED'
                      ? isDark ? 'bg-red-950/40 border-red-500/40' : 'bg-red-50 border-red-200 text-slate-800'
                      : msg.senderType === 'Dispatcher'
                      ? isDark ? 'bg-[#0f1d33]/60 border-emerald-500/30' : 'bg-emerald-50/50 border-emerald-200 text-slate-800'
                      : isDark ? 'bg-[#0e1626]/80 border-[#18253d]' : 'bg-slate-50 border-slate-200 text-slate-800'
                  }`}
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1.5">
                      <span className={`text-[10px] font-bold px-1.5 py-0.2 rounded border ${getSenderBadge(msg.senderType)}`}>
                        {msg.sender}
                      </span>
                      {msg.tacticalStatus && (
                  ...
                  {msg.isVoiceNote ? (
                    <div className="space-y-1.5 mt-1">
                      <div className={`flex items-center gap-1.5 text-[10px] font-mono font-bold px-2 py-0.5 rounded-md w-fit border ${
                        ...
                        <Mic size={10} className={isDark ? "text-emerald-400 animate-pulse" : "text-emerald-600 animate-pulse"} />
          ...
          <div className={`p-2 border-t flex items-center gap-1.5 overflow-x-auto scrollbar-none ${
            ...
            <button
              ...
              className={`px-2 py-0.5 text-[10px] font-semibold border rounded-md shrink-0 transition-colors ${
                isDark ? 'text-emerald-400 bg-emerald-500/10 hover:bg-emerald-500/20 border-emerald-500/20' : 'text-emerald-700 bg-emerald-50 hover:bg-emerald-100 border-emerald-200'
              }`}
            >
              🟢 Expedite Route
            </button>
            ...
            <button
              ...
              className={`px-2 py-0.5 text-[10px] font-semibold border rounded-md shrink-0 transition-colors ${
                isDark ? 'text-red-400 bg-red-500/10 hover:bg-red-500/20 border-red-500/20' : 'text-red-700 bg-red-50 hover:bg-red-100 border-red-200'
              }`}
            >
              ⚠️ Request Backup
            </button>
            <button
              ...
              className={`px-2 py-0.5 text-[10px] font-semibold border rounded-md shrink-0 transition-colors ${
                isDark ? 'text-cyan-400 bg-cyan-500/10 hover:bg-cyan-500/20 border-cyan-500/20' : 'text-blue-700 bg-blue-50 hover:bg-blue-100 border-blue-200'
              }`}
            >
              🩺 Patient Transit
            </button>
            ...
            {isRecordingPTT ? (
              ...
              <form onSubmit={handleSendText} className="flex-1 flex items-center gap-2">
                <input
                  ...
                  className={`flex-1 text-xs rounded-xl px-3 py-2 border focus:outline-none focus:ring-1 focus:ring-emerald-500 ${
                    isDark 
                ...
                <button
                  type="submit"
                  className="p-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white shadow-sm transition-all shrink-0 cursor-pointer"
                  title="Send Message"
                >
                  <Send size={13} />
                </button>
            ...
            <button
              ...
              className={`p-2 rounded-xl border transition-all shrink-0 cursor-pointer ${
                isRecordingPTT
                  ...
                  : isDark
                  ? 'bg-[#15233c] hover:bg-[#1b2f52] text-emerald-400 border-emerald-500/30'
          ...
          <div className={`border rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200 ${
            ...
            <div className={`p-4 border-b flex justify-between items-center ${
              ...
              <h3 className={`text-sm font-bold flex items-center gap-2 ${isDark ? 'text-slate-100' : 'text-slate-900'}`}>
                <ArrowRightLeft size={16} className={isDark ? "text-emerald-400" : "text-emerald-600"} />
            ...
            <form onSubmit={handleCreateInterUnitChannel} className="p-4 space-y-4 text-xs">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <select
                    ...
                    className={`w-full rounded-xl p-2 font-bold focus:ring-1 focus:ring-emerald-500 border ${
              ...
              <div className={`p-3 rounded-xl border text-[11px] space-y-1 ${
                ...
                <div className="flex justify-between font-mono">
                  ...
                  <span className="text-emerald-500 font-bold">256-bit AES P25</span>
                </div>
                <div className="flex justify-between font-mono">
                  <span className={isDark ? "text-slate-200" : "text-slate-700"}>Tactical Channel:</span>
                  <span className={`font-bold ${isDark ? 'text-emerald-400' : 'text-emerald-700'}`}>TAC-{newUnit1}-{newUnit2}</span>
                </div>
              </div>
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button
                  type="submit"
                  className="flex-1 py-2 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl shadow-md transition-all cursor-pointer"
                >
                  Open Tactical Link
                </button>
              </div>
