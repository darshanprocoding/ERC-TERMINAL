...
export const ResponderApp: React.FC<ResponderAppProps> = ({ onReturnToDispatcher }) => {
  ...
  const [activeChatTab, setActiveChatTab] = useState<'incident' | 'direct' | 'general'>('direct');
  ...
  // Smooth scroll message container when messages update or active chat tab changes
  useEffect(() => {
  ...
  useEffect(() => {
    if (isLoggedIn && unitId) {
      ...
      socket.on('responder:receive_message', (data) => {
        ...
        let assignedChannel: 'incident' | 'direct' | 'general' | null = null;

        // Check A: General / Citywide Broadcast
        if (
          targetIncId === 'general' ||
          targetIncId === 'citywide' ||
          targetIncId === 'agency-police' ||
          targetIncId === 'agency-hospitals'
        ) {
          assignedChannel = 'general';
        }
        // Check B: Direct / Private Unit Link (e.g. direct-FE-04, direct-FE-02, unit-fe02-pv23, or targetUnitId is this unit)
        else if (
          targetUnit === myUnit ||
          ...
          targetIncId.startsWith('direct-') ||
          targetIncId.startsWith('unit-')
        ) {
          const isForMe =
            targetUnit === myUnit ||
            unitList.includes(myUnit) ||
          ...
          if (isForMe || isFromDispatcher) {
            assignedChannel = 'direct';
            // If unit is currently undispatched, ensure they switch to direct view
            if (!currentMissionRef.current) {
              setActiveChatTab('direct');
            }
          } else {
            // Private direct message for another unit -> DROP
            return;
          }
        }
        // Check C: Incident-Specific Tactical Channel (e.g. inc-015, INC-2025-015, INC-2026-015)
        else {
          const activeMission = currentMissionRef.current;
          if (activeMission) {
            assignedChannel = 'incident';
        ...
        playRadioChirp(data.isVoiceNote ? 'incoming' : 'incoming');
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (unitId.trim() && password.trim() === '1234') {
      ...
      if (!VALID_UNITS.includes(cleanId)) {
         alert("Invalid Unit ID. Please enter a valid designated callsign (e.g. FE-12, AMB-07, PV-23).");
         return;
      }
      setUnitId(cleanId);
      ...
      alert("Invalid credentials. Try Unit ID (e.g., FE-12, AMB-07) and PIN '1234'");
    }
  };
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    ...
    if (messageInput.trim()) {
      ...
      const targetIncId = activeChatTab === 'incident' 
        ? (currentMission?.id || `direct-${cleanUnit}`) 
  ...
  if (!isLoggedIn) {
    return (
      <div className="min-h-screen bg-[#070b14] flex flex-col items-center justify-center p-4">
        <div className="w-full max-w-sm bg-[#0a0f1d] border border-[#1e2c45] rounded-2xl p-6 shadow-2xl text-center">
           <div className="w-14 h-14 bg-blue-500/10 border border-blue-500/30 rounded-2xl flex items-center justify-center text-blue-400 mx-auto mb-4">
              <Shield size={32} />
           </div>
           <h1 className="text-xl font-bold text-white mb-1">Field Responder Terminal</h1>
           <p className="text-slate-400 text-xs mb-6">Enter vehicle callsign & PIN to connect to ERC Network.</p>
           
           <form onSubmit={handleLogin} className="space-y-4">
             <div>
               <label className="block text-left text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1">Unit Callsign</label>
               <input
                 type="text"
                 placeholder="e.g. FE-12, AMB-07, PV-23, SWAT-01"
                 value={unitId}
                 onChange={e => setUnitId(e.target.value)}
                 className="w-full bg-[#111927] border border-[#1d2a42] rounded-xl px-4 py-2.5 text-white font-mono uppercase focus:outline-none focus:border-blue-500 text-sm"
                 required
               />
             </div>
             <div>
               <label className="block text-left text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1">Tactical Security PIN</label>
               <input
                 type="password"
                 placeholder="1234"
                 value={password}
                 onChange={e => setPassword(e.target.value)}
                 className="w-full bg-[#111927] border border-[#1d2a42] rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-blue-500 text-sm"
                 required
               />
             </div>
             <button type="submit" className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold rounded-xl py-3 shadow-lg shadow-blue-600/30 transition-colors text-sm">
               Connect Unit Terminal
             </button>
           </form>
           
           <div className="mt-5 text-center border-t border-[#1d2b45] pt-4 space-y-3">
             <button
               type="button"
               onClick={handleReturnToERC}
               className="w-full text-xs text-blue-400 hover:text-white bg-blue-500/10 hover:bg-blue-600/30 border border-blue-500/30 rounded-xl py-2 px-3 font-semibold flex items-center justify-center gap-2 transition-all shadow-sm"
             >
               <ArrowLeft size={14} />
             ...
             <div className="text-[11px] text-slate-400">
               Demo PIN: <span className="font-mono text-emerald-400 font-bold">1234</span>
             </div>
  ...
  return (
    <div className="min-h-screen bg-[#070b14] flex flex-col text-slate-200 font-sans">
      {/* Header */}
         ...
         <div className="flex items-center gap-1.5 sm:gap-2">
           ...
           {micState !== 'granted' && (
             <button 
               ...
               className="px-2 sm:px-2.5 py-1.5 rounded-lg border border-blue-500/40 bg-blue-600/20 hover:bg-blue-600/30 text-blue-300 text-xs font-semibold flex items-center gap-1.5 transition-colors animate-pulse shrink-0"
               title="Click to allow Microphone Access for Tactical Radio"
             >
      ...
      {status === 'Maintenance' && (
        <div className="bg-amber-950/70 border-b border-amber-500/40 px-4 py-2.5 flex items-center justify-between animate-in fade-in">
          ...
          <button
            ...
            className="px-3 py-1 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg text-xs font-bold shrink-0 transition-colors shadow-sm ml-2"
          >
            Return to Available
          </button>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-4 max-w-4xl mx-auto w-full">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
             <button 
               onClick={() => updateStatus('Available')}
               className={`p-2.5 sm:p-3 rounded-xl border flex flex-col items-center justify-center gap-1.5 transition-all ${
                 status === 'Available' 
                   ? 'bg-emerald-600 border-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.4)] text-white' 
             ...
             <button 
               ...
               className={`p-2.5 sm:p-3 rounded-xl border flex flex-col items-center justify-center gap-1.5 transition-all ${
                 status === 'En Route' 
                   ? 'bg-blue-600 border-blue-500 shadow-[0_0_15px_rgba(59,130,246,0.4)] text-white' 
             ...
             <button 
               ...
               className={`p-2.5 sm:p-3 rounded-xl border flex flex-col items-center justify-center gap-1.5 transition-all ${
                 status === 'On Scene' 
                   ? 'bg-amber-500 border-amber-400 shadow-[0_0_15px_rgba(245,158,11,0.4)] text-white' 
             ...
             <button 
               ...
               className={`p-2.5 sm:p-3 rounded-xl border flex flex-col items-center justify-center gap-1.5 transition-all ${
                 status === 'Maintenance' 
                   ? 'bg-amber-600 border-amber-400 shadow-[0_0_15px_rgba(217,119,6,0.45)] text-white' 
                   : 'bg-[#111927] border-[#1d2a42] text-slate-400 hover:border-amber-500/40 hover:text-amber-300'
               }`}
             >
        ...
        <div className="bg-[#0b101d] border border-[#172338] rounded-2xl flex flex-col h-80 sm:h-96 overflow-hidden shadow-lg">
           ...
           <div className="flex border-b border-[#172338] bg-[#0d1322] overflow-x-auto scrollbar-none">
             <button 
               ...
               className={`shrink-0 px-3 py-2 text-xs font-bold flex items-center justify-center gap-1.5 transition-colors ${
                 activeChatTab === 'incident' ? 'text-amber-400 border-b-2 border-amber-500 bg-[#111927]' : 'text-slate-400 hover:bg-[#111927]'
               }`}
             >
               <Radio size={13} /> Tactical Net {currentMission ? `(${currentMission.id})` : ''}
               {messages.filter(m => m.channelId === 'incident').length > 0 && (
                 <span className="text-[9px] font-mono px-1 rounded-full bg-amber-500/20 text-amber-300 border border-amber-500/30">
                   {messages.filter(m => m.channelId === 'incident').length}
                 </span>
               )}
             </button>
             <button 
               ...
               className={`shrink-0 px-3 py-2 text-xs font-bold flex items-center justify-center gap-1.5 transition-colors ${
                 activeChatTab === 'direct' ? 'text-blue-400 border-b-2 border-blue-500 bg-[#111927]' : 'text-slate-400 hover:bg-[#111927]'
               }`}
             >
               <RadioTower size={13} /> Direct Link (Dispatch)
               {messages.filter(m => m.channelId === 'direct').length > 0 && (
                 <span className="text-[9px] font-mono px-1 rounded-full bg-blue-500/20 text-blue-300 border border-blue-500/30">
                   {messages.filter(m => m.channelId === 'direct').length}
                 </span>
               )}
             </button>
             <button 
               ...
               className={`shrink-0 px-3 py-2 text-xs font-bold flex items-center justify-center gap-1.5 transition-colors ${
                 activeChatTab === 'general' ? 'text-purple-400 border-b-2 border-purple-500 bg-[#111927]' : 'text-slate-400 hover:bg-[#111927]'
               }`}
             >
               <Globe size={13} /> Citywide Net
               {messages.filter(m => m.channelId === 'general').length > 0 && (
                 <span className="text-[9px] font-mono px-1 rounded-full bg-purple-500/20 text-purple-300 border border-purple-500/30">
                   {messages.filter(m => m.channelId === 'general').length}
                 </span>
               )}
             </button>
           </div>
           ...
           <div ref={responderChatContainerRef} className="flex-1 p-3 overflow-y-auto space-y-2.5 scroll-smooth scrollbar-thin scrollbar-thumb-slate-800">
              {messages.filter(m => m.channelId === activeChatTab).length === 0 ? (
                 <div className="text-center text-slate-500 text-xs mt-10">
                   {activeChatTab === 'incident' ? 'Tactical channel established. Awaiting audio/text transmissions.' : 'Direct dispatch channel open.'}
                 </div>
              ) : (
                 messages.filter(m => m.channelId === activeChatTab).map(msg => (
                   <div key={msg.id} className={`p-2.5 rounded-xl border text-xs ${
                     ...
                     msg.sender === 'You' || msg.sender === 'You (Voice)' ? 'bg-emerald-500/10 border-emerald-500/30' :
                     ...
                     <div className="flex justify-between items-start mb-1">
                        <div className="flex items-center gap-1.5">
                          <span className={`font-bold ${
                            ...
                            msg.sender === 'You' || msg.sender === 'You (Voice)' ? 'text-emerald-400' :
                     ...
                     {msg.isVoiceNote ? (
                       <div className="space-y-1.5 mt-1">
                         <div className="flex items-center gap-1.5 text-[10px] font-mono text-emerald-400 font-bold bg-emerald-950/40 border border-emerald-500/30 px-2 py-0.5 rounded-md w-fit">
                           ...
                           <button
                             ...
                             className={`w-8 h-8 rounded-full flex items-center justify-center transition-all shrink-0 cursor-pointer ${
                               playingVoiceId === msg.id
                                 ? 'bg-emerald-500 text-white animate-pulse shadow-[0_0_12px_#10b981]'
                         ...
                         {msg.text && (
                           <div className="bg-[#0b1322]/80 rounded-lg px-2.5 py-1.5 border border-[#1c2e4a] text-xs text-slate-200">
                             <span className="text-[10px] text-emerald-400 font-mono font-semibold uppercase tracking-wider block mb-0.5">Spoken Message:</span>
             ...
             {isRecordingPTT ? (
               <div className="bg-red-950/90 border border-red-500/60 rounded-xl p-2.5 space-y-2 text-xs text-red-300 shadow-[0_0_20px_rgba(239,68,68,0.3)] animate-in fade-in">
                 ...
                 {liveTranscript ? (
                   <div className="bg-[#080d18] border border-red-500/40 rounded-lg p-2 text-xs text-slate-100 font-mono">
                     <span className="text-[10px] text-emerald-400 font-bold uppercase tracking-wider block mb-0.5">Live Voice Transcribed:</span>
                 ...
                 <div className="flex items-center justify-end gap-2 pt-0.5">
                   ...
                   <button
                     ...
                     className="px-4 py-1.5 rounded-lg bg-red-600 hover:bg-red-500 text-white text-[11px] font-bold transition-colors shadow-[0_0_12px_rgba(239,68,68,0.6)] flex items-center gap-1.5 cursor-pointer"
                   >
                     <Send size={12} />
               ...
               <form onSubmit={handleSendMessage} className="flex items-center gap-2">
                 <input
                   ...
                   placeholder={activeChatTab === 'general' ? 'Citywide broadcasts are read-only...' : "Send tactical text or tap PTT mic..."}
                   disabled={activeChatTab === 'general'}
                   className="flex-1 bg-[#111927] border border-[#1d2a42] rounded-xl px-3 py-2 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-blue-500 disabled:opacity-50"
                 />
                 ...
                 <button
                   ...
                   className="px-3 py-2 rounded-xl text-xs font-bold flex items-center gap-1.5 bg-red-600/20 hover:bg-red-600/30 text-red-300 border border-red-500/40 hover:border-red-500 transition-colors shadow-sm"
                 ...
                 <button 
                   type="submit" 
                   disabled={activeChatTab === 'general' || !messageInput.trim()} 
                   className="bg-blue-600 hover:bg-blue-500 disabled:bg-slate-800 disabled:text-slate-500 text-white rounded-xl px-3.5 py-2 text-xs font-bold transition-colors flex items-center gap-1"
                 >
                   <Send size={13} />
          ...
          <div className="w-full max-w-sm bg-[#0a0f1d] border border-blue-500/40 rounded-3xl p-6 shadow-2xl text-center relative overflow-hidden">
             ...
             <div className="flex items-center justify-center gap-6">
                <button 
                  ...
                  className="w-16 h-16 rounded-full bg-red-600 hover:bg-red-500 text-white flex flex-col items-center justify-center shadow-[0_0_20px_rgba(220,38,38,0.4)]"
                >
                  <PhoneOff size={24} />
                </button>
                <button 
                  ...
                  className="w-16 h-16 rounded-full bg-emerald-600 hover:bg-emerald-500 text-white flex flex-col items-center justify-center shadow-[0_0_20px_rgba(16,185,129,0.4)]"
                >
                  <Phone size={24} />
                </button>
             </div>
      ...
      {inCall && incomingCall && (
        <div className="fixed bottom-0 left-0 w-full bg-emerald-950/90 backdrop-blur-md border-t border-emerald-500/40 p-4 animate-in slide-in-from-bottom flex justify-between items-center z-40">
           ...
           <button 
             ...
             className="px-4 py-2 bg-red-600 hover:bg-red-500 rounded-xl text-white font-bold text-xs"
           >
             End Call
           </button>
        </div>
